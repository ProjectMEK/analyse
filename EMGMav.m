function EMGMav(varargin)
%
% MAV (Mean Absolute Voltage)
%
% Laboratoire GRAME
% Avril 2000, octobre 2010
%
% Fonction qui retourne les valeurs moyennes d'une courbe en fonction du temps.
%
% Le programme utilise un algorithme de "fenêtres glissantes" (fait le calcul de la
% moyenne des valeurs absolues sur chaque fenêtre qui se décalle en fonction de
% l'overlap choisi, l'overlap étant le chevauchement de 2 fenêtres consécutives).
...
%
  ...
  case 'ouverture'
    ...
    fig =figure('Name', 'MAV (Mean Absolute Voltage)', 'Position',lapos, ...
                'CloseRequestFcn','EMGMav(''fermer'')',...
    ...
    uicontrol('Parent',fig, 'tag','NomCanaux', 'BackgroundColor',[1 1 1], 'Position',[posx posy large haut], ...
              'String',hdchnl.Listadname, 'Callback','EMGMav(''chcanal'')', ...
    ...
    uicontrol('Parent',fig, 'tag','ChoixPtDepart', 'Position',[posx posy large-corrx haut], 'String',dep, ...
              'Callback','EMGMav(''Pointdepart'')', 'Style','popupmenu', 'Value',1);
    ...
    uicontrol('Parent',fig, 'Callback','EMGMav(''travail'')', 'Position',[posx haut large haut], 'String','Au travail');
  ...
  case 'travail'
    ...
    case 1    % pt de départ choisi par l'utilisateur
      ...
      for ncan =1:nombre
          ...
          for n =1:nbfen
            b =a+window-1;
            Dtf.Dato.(Dtf.Nom)(n, :) =mean(abs(Dti.Dato.(Dti.Nom)(a:b, :)),1);
            a =a+window-overlap;
            tmp =tmp+1;
          ...
          for j =1:vg.ess
            hdchnl.comment{vg.nad, j} =['MAV.' num2str(window) '-' num2str(overlap) '//' hdchnl.comment{vg.nad, j}];
          end
      	else
          for j = 1:vg.ess
            ...
            for n =1:nbfen
              b =a+window-1;
              Dtf.Dato.(Dtf.Nom)(n, j) =mean(abs(Dti.Dato.(Dti.Nom)(a:b, j)),1);
              a =a+window-overlap;
              tmp =tmp+1;
              waitbar(tmp/total, Wb);
            end
        ...
        hdchnl.adname{vg.nad} =['MAV(' deblank(hdchnl.adname{vg.nad}) ')'];
      end
    %-----
